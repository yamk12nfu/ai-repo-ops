...
- **schema migrations**（`aro upgrade`）— 実装は保留するが、**方針だけは先に
  `docs/plans/00-schema-evolution.md` として書く**（本計画群の merge 後、次の小 PR で追加する）。
  最小の内容: `schema_version` の意味 / minor compatible と major breaking の扱い /
