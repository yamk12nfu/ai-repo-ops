...
- Execution Plan Protocol Stage 1 の authoritative schema、`aro plans check/status/next`、Proposal freshness gate、
  distribution managed schema、consumer の execution-plan allowed path を追加（distribution version 0.1.11）。
...
- `aro guard` の変更ファイル数上限を、project の `ai.max_changed_files` と適用 policy の
  `change_limits.max_changed_files` の小さい方に統一。high-risk policy の上限をproject設定で
  緩和できないようにした。
- managed prompt群の検証境界をguard実装と整合させた。`issue-fix.md` にclean worktree・guard・
  file/line上限を追加し、`knowledge-refresh.md` の既存Knowledge読み取り例外、`review.md` の
  project/policy禁止path和集合とlockfile復旧手順を明確化した。

### Security

- 新規配布する`ai-review.yml`から、現行guardが使用しない`ANTHROPIC_API_KEY`の転送を削除。
  PR番号単位のconcurrencyと`cancel-in-progress`を追加し、high-risk consumer向けにreusable workflowを
  レビュー済み完全SHAへ固定するtrust policyを文書化した。reusable workflowのoptional secret入力は
  後方互換のため維持する。
...
- 正規の`aro sync`が生成したmanaged file / lock変更を`aro guard`が一律`managed_file`・
  `outside_allowed_paths`として拒否していた問題を修正。merge-base上でauthoritative distributionのsyncを
  再実行し、期待bundle全体と一致するpathだけを限定的に認証する。lock-only sync、seed、append patch、
  通常変更とのmixed PRにも対応する。

### Security

- trusted sync認証はPR側lockのchecksumを信頼せず、同一workflow commitのengine source、merge-base lock、
  raw Git blob bytes / modeから再現する。偽造lock、部分commit、patch混入、任意source、policy自己緩和、
  managed削除、symlink/gitlink、実行bit変更はfail-closedで不認証にする。認証時もforbidden/workflow/
  project_config/change limitsは免除しない。

### Changed

- `aro knowledge init` のhuman向け成功出力に、ローカルCodex / Claude Codeへ貼り付ける初回Knowledge
  作成プロンプトと、`knowledge check --strict` → 人間確認・commit → `guard` の次手順を追加。
  実行中のNode entrypoint、対象repoの絶対path、検証済みbase SHAをプロンプト内と後続コマンドへ
  引き継ぎ、PATHにない環境や別directoryからのinitでもコピペ実行できる。JSON出力契約は変更しない。
  READMEにもglobal linkなしの直接実行手順を追加した。
- managed `knowledge-refresh.md` に、初回は変化しにくい正式文書を優先するsource選定基準、PATHなし時の
  launcher再利用、未commit差分をguard対象と誤認しない手順を追加。distribution manifest versionを
  `0.1.3` → `0.1.4` にbumpした。
...
- Repo Knowledge Loop を追加。
  - `aro knowledge init` は必須 `--base <ref>` と HEAD の merge-base にある `.ai/project.yaml` が
    許可した repo にだけ、`index.yaml` と `overview.md` を非上書きで作成する
    （`--dry-run` / `--json` 対応、部分生成時は削除対象と要確認pathを分けて報告）。
  - `aro knowledge check` は index・Markdown・正確な source path・完全 Git SHA・履歴上の provenance・
    source の鮮度を決定的に検証する。通常モードの stale は WARN / exit 0、`--strict` では FAIL / exit 1。
  - authoritative `schemas/knowledge.schema.json`、配布用 managed copy、ローカル Claude Code / Codex
    向け `knowledge-refresh.md` を追加。AI API・外部ネットワーク・自動 PR / merge は使用しない。
...
- reusable `ai-review` workflow と中央 CI の `actions/checkout` / `actions/setup-node` を v5
  （Node.js 24 runtime）へ更新し、guard の実行Node.jsも20から24へ移行してGitHub Actionsの
  deprecation warningを解消。setup-nodeの新しい自動cache挙動は明示的に無効化し、従来の実行条件を維持した。
  配布側 `ai-review.yml` のコメントも現行 guard に合わせ、`anthropic_api_key` は旧AIエンジンとの
  互換性入力として残るだけで登録不要であることを明記した。v5 actionsをself-hosted runnerで使う場合は
  Actions Runner v2.327.1以上が必要（GitHub-hosted runnerは対応済み）。
- 新規 repo の `allowed_paths` と `.gitattributes` に `.ai/local/knowledge/**` を追加。既存 repo は
  設定だけの PR を先に merge してから `aro knowledge init` する二段階導入とした。
- reusable `ai-review` workflow は knowledge index がある repo だけ `aro knowledge check` を実行する。
  通常 PR は非 strict、knowledge を変更する PR は strict。base に存在する index を PR で削除しても
  検査を skip できない。
...
- knowledge source は HEAD と verification commit の両方で通常 Git blob（`100644` / `100755`）かつ
  UTF-8 テキストの正確な repo 相対 path に限定し、glob・traversal・symlink・未追跡ファイル・
  `.env*`・`secrets/**`・`.git/**`・`.ai/**`・依存物・build生成物を内容読込前に拒否する。
  禁止patternはrepo rootだけでなく、すべてのnested階層へ適用する。
  verification commit は存在し、HEAD の祖先で、source がその commit に存在することを要求する。
